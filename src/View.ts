...
export class GameView {
    ...
    private planeAnimation!: PIXI.AnimatedSprite;
    private graphLine!: PIXI.Graphics;
    private controlPanel!: PIXI.Container;
    private plusButton!: PIXI.Sprite;
    private minusButton!: PIXI.Sprite;
    private betText!: PIXI.Text;

    private containerId: string;
    private readonly maxHorizontalPath: number = 400;

    // Callbacks
    public onPlaceBet: () => void = () => { };
    ...
    public async init(assets: string[]) {
        await this.app.init({
            width: 800,
            height: 450,
    ...
    private createButton(text: string, color: number, onClick: () => void): PIXI.Container {
        ...
        (container as any).setText = (newText: string) => buttonText.text = newText;
        (container as any).setColor = (newColor: number) => {
            background.clear();
        ...
        switch (model.GamePhase) {
            case GamePhase.WAITING:
                ...
                if (model.PlayerHasBet) {
                    (this.mainButton as any).setText('WAITING...');
                    (this.mainButton as any).setColor(0x888888);
                    this.mainButton.children[0].removeAllListeners('pointerdown');
                } else {
                    betButtonsActive = true;
                    (this.mainButton as any).setText('PLACE BET');
                    (this.mainButton as any).setColor(0x4CAF50);
                    this.mainButton.children[0].removeAllListeners('pointerdown');
            ...
            case GamePhase.PLAYING:
                ...
                if (model.PlayerHasBet) {
                    if (model.PlayerHasCashedOut) {
                        ...
                        (this.mainButton as any).setText('CASHEd OUT');
                        (this.mainButton as any).setColor(0x888888);
                        this.mainButton.children[0].removeAllListeners('pointerdown');
                    } else {
                        this.resultText.text = '';
                        (this.mainButton as any).setText(`CASH OUT`);
                        (this.mainButton as any).setColor(0xFF4500);
                        this.mainButton.children[0].removeAllListeners('pointerdown');
                        this.mainButton.children[0].on('pointerdown', () => this.onCashOut());
                    }
                } else {
                    this.resultText.text = 'Round in progress...';
                    (this.mainButton as any).setText('---');
                    (this.mainButton as any).setColor(0x888888);
                    this.mainButton.children[0].removeAllListeners('pointerdown');
                }
                break;
    ...
    private drawGraph(multiplier: number) {
    ...
    private updatePlanePosition(multiplier: number, isPlaying: boolean) {
    ...
    public getAppTicker(): PIXI.Ticker {
